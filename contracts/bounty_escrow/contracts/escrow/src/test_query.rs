...
#![cfg(test)]
extern crate std;
use crate::{BountyEscrowContract, BountyEscrowContractClient, EscrowStatus, EscrowFilter, Pagination};
...
fn create_test_env<'a>(env: &'a Env) -> (BountyEscrowContractClient<'a>, Address, Address, token::Client<'a>, token::StellarAssetClient<'a>) {
    env.mock_all_auths();
...
fn test_get_bounties_filtering() {
    ...
    let bounds = Pagination { start_index: 0, limit: 10 };
...
fn test_pagination() {
    ...
    let filter_none = EscrowFilter {
        status: None, depositor: None, min_amount: None, max_amount: None, start_time: None, end_time: None
    };
    
    // Page 1: 2 items
    let page1 = client.get_bounties(&filter_none, &Pagination { start_index: 0, limit: 2 });
    assert_eq!(page1.len(), 2);
    assert_eq!(page1.get(0).unwrap().0, 1);
    assert_eq!(page1.get(1).unwrap().0, 2);
    
    // Page 2: 2 items (skip 2)
    let page2 = client.get_bounties(&filter_none, &Pagination { start_index: 2, limit: 2 });
    assert_eq!(page2.len(), 2);
    assert_eq!(page2.get(0).unwrap().0, 3);
    assert_eq!(page2.get(1).unwrap().0, 4);
    
    // Page 3: 1 item (skip 4)
    let page3 = client.get_bounties(&filter_none, &Pagination { start_index: 4, limit: 2 });
...
fn test_large_dataset_pagination() {
    ...
    let page = client.get_bounties(&filter_none, &Pagination { start_index: 3, limit: 3 });
    assert_eq!(page.len(), 3);
    assert_eq!(page.get(0).unwrap().0, 4); 
    assert_eq!(page.get(2).unwrap().0, 6);
    
    // Query end of list
    let last_page = client.get_bounties(&filter_none, &Pagination { start_index: 8, limit: 5 });
